...
class Newsletter(BasePage):
    ...
    def confirm_newsletter_sticky_subscription(self):
        return self.driver.find_element(*PageLocators.newsletter_sticky_subscribed_msg)

    def confirm_newsletter_homepage_subscription(self):
        return self.driver.find_element(*PageLocators.newsletter_homepage_subscribed_msg)
    ...
    def enter_email_in_sticky(self, email):
        time.sleep(1)
        email_field = self.driver.find_element(*PageLocators.newsletter_sticky_email_field)
        email_field.send_keys(email)
    ...
    def get_email_message_error_in_sticky(self):
        email_error = self.driver.find_element(*PageLocators.newsletter_sticky_error_email).get_attribute("innerHTML")
        return email_error
    ...
    def submit_newsletter_sticky_form(self):
        self.driver.click_to_element(PageLocators.newsletter_sticky_subscribe_cta)
        time.sleep(1)
