...
class Header(BasePage, BasePageAPI):
    ...
    def click_cta_all_product_updates(self, locale, viewport):
        if locale == '/' and viewport == 'desktop':
            self.driver.click_to_element(
                PageLocators.menu_all_product_updates_cta_desktop
            )
        elif locale == '/' and viewport == 'mobile':
            self.driver.click_to_element(
                PageLocators.menu_all_product_updates_cta_mobile
            )
    ...
    def click_on_kebab_option(self, kebab_option):
        self.driver.click_to_element(
            PageLocators.kebab__options_locators[kebab_option]
        )

    def click_on_submenu_item(self, submenu, get_viewport):
        if get_viewport == 'mobile':
            submenu_locator = self.get_item_selector(
                submenu, PageLocators.submenu_locators_mobile
            )
            self.driver.click_to_element(submenu_locator)
        elif get_viewport == 'desktop':
            submenu_locator = self.get_item_selector(
                submenu, PageLocators.submenu_locators_desktop
            )
            self.driver.click_to_element(submenu_locator)
    ...
    def click_on_subscribe_cta_mobile(self):
        time.sleep(2)
        self.driver.click_to_element(
            PageLocators.menu_subscribe_mobile_hamburger_cta
        )

    def click_on_subscribe_cta_mobile_sticky(self):
        time.sleep(2)
        self.driver.click_to_element(
            PageLocators.menu_subscribe_mobile_cta_sticky
        )
    ...
    def click_see_all_cta_company_sub_menu(self, viewport):
        if viewport == 'desktop':
            see_all_items = self.driver.get_urls_list(
                PageLocators.submenu_company_news_see_all_ctas
            )
            self.check_internal_status(see_all_items)
        elif viewport == 'mobile':
            see_all_items = self.driver.get_urls_list(
                PageLocators.submenu_company_news_see_all_ctas_mobile
            )
            self.check_internal_status(see_all_items)
    ...
    def confirm_kebab_menu_opts(self, language, option):
        kebab_option_selector = self.get_item_selector(
            option, PageLocators.kebab__options_locators
        )
        option_text_in_page = self.driver.find_element(
            *kebab_option_selector
        ).text
        self.logger.info('%s option_text_in_page', option_text_in_page)
        option_text_expected = self.get_kebab_opt_expected_text(
            language, option
        )
    ...
    def get_sitespaces_list(self):
        self.close_bar(PageLocators.cookie_banner_ok_cta)
        site_spaces_list = self.driver.find_elements(
            *PageLocators.site_spaces_in_ads_and_analytics
        )
    ...
    def get_random_sitespace(self, site_spaces_list, get_viewport):
        self.random_site_space_index = self.get_random_index_in_list(
            site_spaces_list, get_viewport
        )
    ...
    def get_sitespace_title_expected_in_products(self):
        ...
        return self.get_element_in_list(
            Constants.SITESPACE_TITLE_IN_PRODUCTS, self.random_site_space_index
        )

    def get_sitespace_title_expected_in_nav(self):
        return self.get_element_in_list(
            Constants.SITESPACE_TITLE_IN_NAV_MENU, self.random_site_space_index
        )

    def get_site_space_title_in_navigation(self):
        title_in_sitespace = (
            self.driver.find_element(*PageLocators.site_space_title_in_nav_menu)
    ...
    def get_publish_date_in_rss(self):
        ...
        date_formatted = self.get_date_in_api_format(
            publish_date[5:16], Constants.DATE_FORMAT_IN_RSS
        )
    ...
    def click_submenu_items_mobile(self, submenu):
        locator = self.get_item_selector(
            submenu, PageLocators.submenu_items_mobile
        )
