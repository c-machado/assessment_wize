...
class Feed(BasePage, BasePageAPI):
    ...
    def get_articles_in_feed_list(self):
        ...
        articles_list = self.driver.find_elements(
            *PageLocators.feed_articles_list
        )
    ...
    def get_article_dates_in_feed_list(self):
        ...
        article_dates = self.driver.find_elements(
            *PageLocators.feed_article_dates_list
        )
        article_dates_in_feed = []
        for article_date in article_dates:
            article_dates_in_feed.append(
                article_date.get_attribute('innerHTML')
            )
        return article_dates_in_feed

    def get_article_dates_in_feed_with_year(self):
        article_dates_list = self.driver.find_elements(
            *PageLocators.feed_article_dates_list
        )
        dates_with_year_list = []
        for date in article_dates_list:
            date_with_year = date.get_attribute('outerHTML').split(
                'datetime="'
            )[1]
    ...
    def get_article_titles_in_feed(self):
        article_titles_list = self.driver.find_elements(
            *PageLocators.feed_article_titles_list
        )
        titles_list = []
        for article_title in article_titles_list:
            if self.contains_ampersand_char(
                article_title.get_attribute('innerHTML')
            ):
                titles_list.append(
                    self.replace_ampersand_char(
                        article_title.get_attribute('innerHTML')
                    )
                )
                self.logger.info(
            ...
            elif self.contains_hyphen_char(
                article_title.get_attribute('innerHTML')
            ):
                titles_list.append(
                    self.replace_hyphen_char(
                        article_title.get_attribute('innerHTML')
                    )
                )
                self.logger.info(
                ...
                self.logger.info(
                    '%s no update', article_title.get_attribute('innerHTML')
                )
        return titles_list
    ...
    def get_date_article_in_feed(self):
        ...
        for article_date_index in range(0, len(article_date_list)):
            ...
            if article_date_index == self.random_article:
                self.logger.info(
                    '%s article_date_index chosen randomly', article_date_index
                )
                self.logger.info(
                    '%s actual_article_date_in_feed_to_compare',
    ...
    def get_date_first_article_in_feed(self, keyword_url):
        ...
        date_in_first_article = self.get_article_dates_in_latest_api(
            keyword_url
        )[0]
    ...
    def get_date_from_article_in_feed_in_latest_api(self, keyword_url):
        ...
        for index in range(start_range, len(article_dates_in_api)):
            ...
            self.logger.info(
                '%s date expected in api format:', article_dates_in_api[index]
            )
            if index == random_temp:
                self.logger.info('%s random temp', random_temp)
                self.logger.info('%s index api', index)
                self.logger.info(
                    '%s date expected in api format:',
                    article_dates_in_api[index],
                )
                self.article_date_in_api = article_dates_in_api[index]
                return self.article_date_in_api

    def get_date_article_in_feed_per_locale_and_year_babel(
        self, locale, date_article_in_api
    ):
        ...
        year_current_article = self.get_year_in_given_date(
            date_article_in_api, Constants.DATE_FORMAT_IN_API
        )
        current_year = datetime.datetime.now().year
        date_in_api = datetime.datetime.strptime(
            date_article_in_api, '%Y-%m-%d'
        )
        if current_year == year_current_article:
            ...
            date_in_feed = self.get_date_in_babel_format_previous_year(
                date_in_api, locale
            )
            return date_in_feed
    ...
    def get_date_in_babel_format(self, date, locale):
        ...
        if locale == 'de_DE':
            ...
            date_in_babel_format = f'{month} {day}'
        elif locale in ('fr_CA', 'pt_BR', 'it_IT', 'en_GB'):
            date_in_babel_format = f'{day} {month[:3].capitalize()}'
        else:
            date_in_babel_format = format_date(
                date, format=current_locale, locale=locale
            ).capitalize()
    ...
    def get_date_in_babel_format_previous_year(self, date, locale):
        ...
        current_locale = (
            Constants.DATE_FORMAT_BABEL_IN_FEED_PAST_YEAR_PER_LOCALE[locale]
        )
        if locale == 'de_DE':
            date_in_babel_format = f'{month[:3]}. {year}'
        elif locale == 'en_africa':
            date_in_babel_format = f'{month} {year}'
        elif locale in ('fr_CA', 'pt_BR', 'it_IT', 'en_GB', 'en_AU', 'es_ES'):
            date_in_babel_format = f'{month[:3].capitalize()} {year}'
        else:
            date_in_babel_format = format_date(
                date, format=current_locale, locale=locale
            ).capitalize()
    ...
    def get_date_in_article_in_babel_format(self, locale):
        ...
        date_in_article = datetime.datetime.strptime(
            self.article_date_in_api, Constants.DATE_FORMAT_IN_API
        )
        year = self.get_year_in_given_date_babel_format(date_in_article, locale)
        current_year = str(datetime.datetime.now().year)
        if year == current_year:
            return self.get_date_in_article_current_year(
                locale, date_in_article
            )
        else:
            return self.get_date_in_article_previous_year(
                locale, date_in_article
            )

    def get_date_in_article_current_year(self, locale, date_in_article):
        month = self.get_month_in_given_date_babel_format(
            date_in_article, locale
        )
        ...
        if locale == 'de_DE':
            ...
            date__article_in_babel_format = f'{month} {day} {year}'
        elif locale in ('fr_CA', 'pt_BR', 'it_IT', 'en_GB'):
            date__article_in_babel_format = (
                f'{day} {month[:3].capitalize()}, {year}'
            )
        else:
            current_locale = Constants.DATE_FORMAT_PER_LOCALE_BABEL[locale]
            date__article_in_babel_format = format_date(
                date_in_article, format=current_locale, locale=locale
            ).capitalize()
        self.logger.info(
            '%s date__article_in_babel_format - current year ',
            date__article_in_babel_format,
        )
    ...
    def get_date_in_article_previous_year(self, locale, date_in_article):
        ...
        self.logger.info(
            '%s date__article_in_babel_format - previous year',
            date__article_in_babel_format,
        )
        return date__article_in_babel_format
    ...
    def click_to_random_article_in_feed(self, keyword, get_viewport):
        ...
        self.logger.info(
            '%s random_article in click article_index:', self.random_article
        )
        ...
        if length_list >= 3:
            ...
            self.logger.info(
                ...
                self.driver.find_element(
                    *PageLocators.feed_load_more_text
            ...
            if len(self.get_articles_in_feed_list()) >= length_list + 1:
                self.logger.info(
                    '%s length list', len(self.get_articles_in_feed_list())
                )
                return True
    ...
    def confirm_articles_in_feed_homepage(self, keyword_url):
        ...
        print(
            'all', all(item in headlines_in_api for item in headlines_in_feed)
        )
    ...
    def confirm_tagging_in_feed_articles(self, keyword_url, get_viewport):
        ...
        primary_tags_articles_in_feed = self.get_article_tags_in_latest_api(
            keyword_url
        )
    ...
    def get_secondary_tags(self, keyword_url, get_viewport):
        ...
        secondary_tags = ArticlePage(
            self.driver
        ).get_secondary_tags_in_article_api_format()
        self.logger.info('%s secondary_tags', secondary_tags)
        return secondary_tags

    def get_tag_articles_in_feed(self):
        tags_list = self.driver.find_elements(
            *PageLocators.search_eyebrow_articles_in_feed
        )
        tags = []
        print(len(tags_list))
        for index, element in enumerate(tags_list):
            tag_eyebrow = self.remove_html_tags(
                element.get_attribute('innerHTML')
            )
            tag_eyebrow_principal = tag_eyebrow.split('/ ')[1]
            self.logger.info(
                '%s tag before replacing space', tag_eyebrow_principal
            )
            tag_eyebrow_principal = self.replace_space(
                tag_eyebrow_principal.lower()
            )
            self.logger.info(
                '%s tag after replacing space', tag_eyebrow_principal
            )
            tags.append(tag_eyebrow_principal)
        return tags

    def get_articles_indexes_matching_sitespace_tag(
        self, eyebrow_in_articles, sitespace
    ):
    ...
    def click_on_sitespace_element(self, index, keyword, get_viewport):
        ...
        self.driver.find_element(
            By.CSS_SELECTOR,
            '.feed-article.ng-scope:nth-child(' + '% s' % index + ')',
        ).click()
