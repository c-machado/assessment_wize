...
class Search(BasePage, BasePageAPI):
    ...
    def click_search_icon_in_nav_bar(self):
        ...
        self.driver.wait_for_element_visible(
            *PageLocators.search_bar_text_field
        )
        time.sleep(1)
    ...
    def click_filter_by_random_option(self):
        ...
        self.tag_to_filter = filter_by_options_list[
            self.random_filter
        ...
        self.driver.wait_for_page_load()
        self.driver.wait_for_element_visible(
            PageLocators.search_tag_filter_selected
        )
    ...
    def click_on_collection_element(self, index):
        ...
        self.driver.find_element(
            By.CSS_SELECTOR,
            '.uni-search-results__list > a:nth-child(' + index + ')',
        ).click()
    ...
    def get_msg_no_search_results_per_language(text_to_search, language):
        for language_id, message in Constants.SEARCH_NO_RESULTS_MSG.items():
            if language_id == language:
                message_no_search_results = re.sub(
                    'text_to_search', text_to_search, message
                )
    ...
    def get_msg_no_search_results_in_page(self):
        print(
            'actual msg',
            self.driver.find_element(
                *PageLocators.search_no_results_header
            ).get_attribute('innerHTML'),
        )
    ...
    def get_list_filter_by_results(self):
        self.driver.wait_for_page_load()
        filter_by_options = self.driver.find_elements(
            *PageLocators.search_filter_by
        )
    ...
    def get_results_filtered(self, keyword, get_viewport):
        ...
        articles_tags_in_feed_results = (
            self.get_tag_eyebrow_in_feed_results_page(
                articles_in_feed, keyword, get_viewport
            )
        )
    ...
    def get_suggested_results_expected(self, keyword_url, text_to_search):
        suggested_results_api = self.get_suggested_results_in_search_api(
            keyword_url, text_to_search
        )
    ...
    def get_search_results_in_page(self):
        results_in_page = self.driver.find_elements(
            *PageLocators.search_results_list
        )
        headlines_in_results_page = self.get_search_results_headlines(
            results_in_page
        )
        self.logger.info(
            '%s headlines_in_results_page', headlines_in_results_page
        )
    ...
    def get_suggested_results_in_page(self):
        suggested_results_in_page = self.driver.find_elements(
            *PageLocators.search_suggestions_results_list
        )
        headlines_suggested_results = self.get_search_results_headlines(
            suggested_results_in_page
        )
        self.logger.info(
            '%s headlines_suggested_results', headlines_suggested_results
        )
        return headlines_suggested_results

    def get_tag_eyebrow_in_feed_results_page(
        self, eyebrow_in_articles, keyword, get_viweport
    ):
        ...
        for element in eyebrow_in_articles:
            ...
            tag_eyebrow_principal = (
                tag_eyebrow.split('/ ')[1]
                if not (keyword.__contains__('ar-mena'))
                else tag_eyebrow.split('- ')[1]
                + '- '
                + tag_eyebrow.split('- ')[2]
            )
            ...
            if 'collection' in tag_eyebrow_principal:
                collection_index += 1
                self.collections_dict.setdefault(collection_index, []).append(
                    index
                )
                self.collections_dict.setdefault(collection_index, []).append(
                    element
                )
            elif not (tag_eyebrow_principal.strip().startswith('From')) and (
                tag_eyebrow_principal != self.tag_to_filter
            ):
                tag_eyebrow_secondary = (
                    tag_eyebrow.split('/ ')[2]
                    if not (keyword.__contains__('ar-mena'))
                    else tag_eyebrow.split('- ')[1]
                    + '- '
                ...
                if tag_eyebrow_principal.strip().startswith('From'):
                    tag_articles_eyebrow.append(
                        tag_eyebrow_principal[
                            5 : len(self.tag_to_filter) + 6
                        ].strip()
                    )
                else:
    ...
    def get_collection_primary_tag(self, keyword, get_viewport):
        ...
        for item in self.collections_dict.values():
            ...
            data_analytics = self.driver.find_element(
                *PageLocators.collection_data_analytics
            )
            primary_tag = (
                data_analytics.get_attribute('outerHTML').split('primaryTag')[1]
            ).split('&quot;')[2]
    ...
    def is_search_bar_not_visible(self):
        self.driver.wait_for_element_not_visible(
            *PageLocators.search_bar_text_field
        )

    def is_searchbar_button_visible(self):
        return self.driver.find_element(
            *PageLocators.search_icon_nav_desktop
        ).is_displayed()
    ...
    def is_search_results_header_visible(self):
        self.logger.info(
            '%s search_bar',
            self.driver.find_element(
                *PageLocators.search_results_header
