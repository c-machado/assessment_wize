...
@when(parsers.parse("the user chooses the {filter_by_type} and {filter_by_product}"))
...
@then(parsers.parse("the system updates the grid with {type_filter} in {keyword}"))
...
@then(parsers.parse("the system shows the grid with the {type_filter} and {tag_filter} in {keyword}"))
def assets_get_filtered_by_type_and_tag(press, type_filter, tag_filter, keyword):
    actual_titles = press.get_titles_in_press_assets_page()
    print('actual_titles', actual_titles)
