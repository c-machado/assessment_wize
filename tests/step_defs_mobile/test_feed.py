...
scenarios(
    '../features/feed_latest_news/feed_load_more_stories.feature',
...
@when(
    parsers.parse(
        'the user confirm the format date in the feed corresponding to {keyword} {locale}'
    )
)
def user_check_date_in_feed(keyword, locale, feed, driver, base_page):
    ...
    date_article_in_api = feed.get_date_from_article_in_feed_in_latest_api(
        keyword
    )
    expected_date_in_feed = (
        feed.get_date_article_in_feed_per_locale_and_year_babel(
            locale, date_article_in_api
        )
    )
...
@when(
    parsers.parse('the user clicks on load more stories cta on {keyword} feed')
)
...
@then(
    parsers.parse('the tags associated match with the content in the {keyword}')
)
