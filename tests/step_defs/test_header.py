...
def redirected_to_page_url(url, base_page, driver):
    assert base_page.get_status_redirect() == 200
...
@pytest.mark.flaky(
    reruns=1,
    reruns_delay=0.5,
    reason="requests.exceptions.ConnectionError: ('Connection aborted.',"
...
def check_http_status_internal_links(header, submenu):
    submenu_items = header.get_url_submenu_items(
        submenu, PageLocators.submenu_items_urls
    )
...
@when(
    parsers.parse(
        'the user clicks in an article in a {sitespace_tag} in {keyword}'
    )
)
def user_clicks_on_random_site_space(
    sitespace_tag, keyword, feed, get_viewport
):
    tags_list = feed.get_eyebrows_in_feed_site_space_page()
    articles_within_sitespace = (
        feed.get_articles_indexes_matching_sitespace_tag(
            tags_list, sitespace_tag
        )
    )
    random_index = feed.get_random_index_in_list(
        articles_within_sitespace, get_viewport
    )
    feed.click_on_sitespace_element(
        articles_within_sitespace[random_index], keyword, get_viewport
    )


@then(
    parsers.parse(
        'the system shows the {sitespace_title} nav menu in an article'
    )
)
...
@when(
    parsers.parse(
        'the user on {keyword} clicks the CTA See all product updates'
    )
)
